...
class Pm25Dataset(object):
    ...
    def generateinput(self):
            ...
            if os.path.exists(gfsdir+filename) and os.path.getsize(gfsdir+filename)>0:#判断文件是否存在
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                    ...
                    for k in range(self.n_points):
                        inputs[(0*self.n_points+k,(h+24)*2+cnt)]=interp(temp.reshape((180*4+1,360*4)),self.lat_x[k],self.lon_y[k])
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                    for k in range(self.n_points):
                        inputs[(0*self.n_points+k,(h+24)*2+cnt)]=inputs[(0*self.n_points+k,(h+24)*2+cnt-6)]
                        #(h+24)*2+cnt是用来找对应的格位置，（h+24）*2是对应小时的6格开始位置，再加上cnt作为偏置
                cnt=cnt+1
        ...
        for i in range(1,self.n_perpoint):#填上矩阵中，剩余trainning example的数据
            ...
            for k in range(self.n_points):
                inputs[i*self.n_points+k,0:6*8+t_predict*2-6]=inputs[(i-1)*self.n_points+k,6:6*8+t_predict*2]
            ...
            if os.path.exists(gfsdir+filename) and os.path.getsize(gfsdir+filename)>0:#判断文件是否存在
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                    temp=cPickle.load(f)
                    for k in range(self.n_points):
                        inputs[(i*self.n_points+k,6*8+t_predict*2-6+cnt)]=interp(temp.reshape((180*4+1,360*4)),self.lat_x[k],self.lon_y[k])
                        #(h+24)*2+cnt是用来找对应的格位置，（h+24）*2是对应小时的6格开始位置，再加上cnt作为偏置
                    cnt=cnt+1
                f.close()
            else:#用三小时之前的替换
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                    for k in range(self.n_points):
                        inputs[(i*self.n_points+k,6*8+t_predict*2-6+cnt)]=inputs[(i*self.n_points+k,6*8+t_predict*2-12+cnt)]
                        #(h+24)*2+cnt是用来找对应的格位置，（h+24）*2是对应小时的6格开始位置，再加上cnt作为偏置
                cnt=cnt+1
        ...
        for h in range(24+t_predict):#在starttime前后两天的时间点,这个循环填上第一个example的后24+t_predict个数据
            ...
            if int(name) > 2015061324:
                if os.path.exists(pm25dir+name[0:8]+'/'+name+'.pkl.gz') and os.path.getsize(pm25dir+name[0:8]+'/'+name+'.pkl.gz')>0:#判断文件是否存在
                    ...
                    f.close()
                    for k in range(self.n_points):
                        inputs[(0*self.n_points+k,6*8+t_predict*2+h)]=temp[(self.cord_x[k],self.cord_y[k])]-pm25mean[int(name[8:10])][self.cord_x[k],self.cord_y[k]]
                    #(h+24)*2+cnt是用来找对应的格位置，（h+24）*2是对应小时的6格开始位置，再加上cnt作为偏置
                else:#用1小时之前的替换
                    for k in range(self.n_points):
                        inputs[(0*self.n_points+k,6*8+t_predict*2+h)]=inputs[(0*self.n_points+k,6*8+t_predict*2+h-1)]                
                cnt=cnt+1
            else:
        ...
        for i in range(1,self.n_perpoint):#for 全部行,生成后24+t_predict维数据
            ...
            for k in range(self.n_points):
                inputs[i*self.n_points+k,6*8+t_predict*2:(6*8+t_predict*2)+(24+t_predict)-3]=inputs[(i-1)*self.n_points+k,(6*8+t_predict*2)+3:(6*8+t_predict*2)+(24+t_predict)]
            for h in range(3):#最后新的3位数据要读文件
                ...
                if int(name) > 2015061324:
                    if os.path.exists(pm25dir+name[0:8]+'/'+name+'.pkl.gz') and os.path.getsize(pm25dir+name[0:8]+'/'+name+'.pkl.gz')>0:#判断文件是否存在
                        ...
                        f.close()
                        for k in range(self.n_points):
                            inputs[i*self.n_points+k,(6*8+t_predict*2)+(24+t_predict)-3+h]=temp[self.cord_x[k],self.cord_y[k]]-pm25mean[int(name[8:10])][self.cord_x[k],self.cord_y[k]]
                    else:#用右边后一个小时的数据填充（暂定可能的补偿方法）
                        for k in range(self.n_points):
                            inputs[i*self.n_points+k,(6*8+t_predict*2)+(24+t_predict)-3+h]=inputs[i*self.n_points+k,(6*8+t_predict*2)+(24+t_predict)-3+h-1]
                else:
                    if os.path.exists(pm25dir+name+'.pkl.gz') and os.path.getsize(pm25dir+name+'.pkl.gz')>0:#判断文件是否存在
                        f = gzip.open(pm25dir+name+'.pkl.gz', 'rb')
                        print(name+'.pkl.gz')
        ...
        for i in range(inputs.shape[0]):
            ...
            if np.mean(test)>18 and np.var(test)>2200:
...
if __name__ == '__main__':
    ...
    savefile(obj.input_data,savedir+'120hDetachValid'+today.strftime('%Y%m%d')+'_t100p100.pkl.gz')
    ...
    print "saved at "+savedir+'120hDetachValid'+today.strftime('%Y%m%d')+'_t100p100.pkl.gz'
