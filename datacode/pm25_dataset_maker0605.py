...
class Pm25Dataset(object):
    ...
    '''
    def __init__(self,lon=np.hstack((np.array([116.3883,117.20,121.48,106.54,118.78,113.66]),110+10*np.random.rand(94))),lat=np.hstack((np.array([39.3289,39.13,31.22,29.59,32.04,34.76]),32+10*np.random.rand(94))),start='2015040108',stop='2015051008',d_in=168,d_out=48):
    ...
    def generateinput(self):
        ...
        '''gfs,144 dimentions'''
        for h in range(-24,48,3):#在starttime前后两天的时间点,这个循环填上第一个example的前144个数据
        ...
        for i in range(1,self.n_perpoint):#填上矩阵中，剩余trainning example的数据
            ...
            for k in range(self.n_points):
                inputs[i+k*self.n_perpoint,0:138]=inputs[i+k*self.n_perpoint-1,6:144]
            #对于当前current所指的这一行，最后小时的六个数据需要以下重新读文件获得
            p_time=current+datetime.timedelta(hours=45)
            #p_time就代表这一帧的时间，尝试打开对应文件
            if p_time.hour==3 or p_time.hour==9 or p_time.hour==15 or p_time.hour==21:#去开003的数据
                name=p_time-datetime.timedelta(hours=3)
                filename=name.strftime('%Y%m%d')+'_'+name.strftime('%H')+'_003.pkl.gz'
            ...
            if os.path.exists(gfsdir+filename) and os.path.getsize(gfsdir+filename)>0:#判断文件是否存在
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                    temp=cPickle.load(f)
                    for k in range(self.n_points):
                        inputs[(i+k*self.n_perpoint,138+cnt)]=interp(temp.reshape((180*4+1,360*4)),self.lat_x[k],self.lon_y[k])
                        #(h+24)*2+cnt是用来找对应的格位置，（h+24）*2是对应小时的6格开始位置，再加上cnt作为偏置
                    cnt=cnt+1
                f.close()
            else:#用三小时之前的替换
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                    for k in range(self.n_points):
                        inputs[(i+k*self.n_perpoint,138+cnt)]=inputs[(i+k*self.n_perpoint,84+cnt)]
                        #(h+24)*2+cnt是用来找对应的格位置，（h+24）*2是对应小时的6格开始位置，再加上cnt作为偏置
                cnt=cnt+1
        ...
        for h in range(72):#在starttime前后两天的时间点,这个循环填上第一个example的后72个数据
            ...
            if os.path.exists(pm25dir+name+'.pkl.gz') and os.path.getsize(pm25dir+name+'.pkl.gz')>0:#判断文件是否存在
                ...
                f.close()
                for k in range(self.n_points):
                    inputs[(0+k*self.n_perpoint,144+h)]=temp[(self.cord_x[k],self.cord_y[k])]-pm25mean[int(name[8:10])][self.cord_x[k],self.cord_y[k]]
                #(h+24)*2+cnt是用来找对应的格位置，（h+24）*2是对应小时的6格开始位置，再加上cnt作为偏置
            else:#用3小时之前的替换
                for k in range(self.n_points):
                    inputs[(0+k*self.n_perpoint,144+h)]=inputs[(0+k*self.n_perpoint,144+h-1)]                
            cnt=cnt+1
            
        for i in range(1,self.n_perpoint):#for 全部行,生成后72维数据
            current=self.starttime+datetime.timedelta(hours=3*i)
            #对于当前current所指的这一行，前90维的数据可以从上一行获得
            for k in range(self.n_points):
                inputs[i+k*self.n_perpoint,144:144+69]=inputs[i+k*self.n_perpoint-1,144+3:144+72]
            for h in range(3):#最后新的3位数据要读文件
                ...
                name=(current+datetime.timedelta(hours=h+46)).strftime('%Y%m%d%H')#未来46，47，48小时
                if os.path.exists(pm25dir+name+'.pkl.gz') and os.path.getsize(pm25dir+name+'.pkl.gz')>0:#判断文件是否存在
                    f = gzip.open(pm25dir+name+'.pkl.gz', 'rb')
                    print(name+'.pkl.gz')
                    temp=cPickle.load(f)
                    f.close()
                    for k in range(self.n_points):
                        inputs[i+k*self.n_perpoint,144+69+h]=temp[self.cord_x[k],self.cord_y[k]]-pm25mean[int(name[8:10])][self.cord_x[k],self.cord_y[k]]
                else:#用右边后一个小时的数据填充（暂定可能的补偿方法）
                    for k in range(self.n_points):
                        inputs[i+k*self.n_perpoint,144+69+h]=inputs[i+k*self.n_perpoint,144+69+h-1]
